...
async function register(req, res) {
   ...
   try {
    ...
    if (user.length > 0) {
        return res.status(StatusCode.BAD_REQUEST).json({ message: 'Username or email already exists' });
    }
   if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(password)) {
    ...
    return res.status(StatusCode.CREATED).json({ message: 'User registered successfully' });
   }
    catch (error) {
